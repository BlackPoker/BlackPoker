...
リゾルブ項目には「即時」または「ステージ」のいずれかが設定されます。
...
**[6-8] バッファにリゾルブ=ステージはあるか？**  
    バッファ内にリゾルブがステージのリクエストが存在するかを判定します。  
    存在する場合、
    ターンプレイヤーから順にステージの処理グループへ進みます。

**[6-9] 該当プレイヤーにリゾルブ=ステージがあるか？**  
    現在処理対象となっているプレイヤーがコントローラーとなっているリクエストがバッファにあるかを確認します。  
    リゾルブがステージのリクエストが存在するかを判定し、
    存在しない場合は、そのプレイヤーでの処理を終了し、次のプレイヤーへ移行します。

**[6-10] タイミング=メインのステージアクションを処理**  
    該当プレイヤーについて、タイミングが「メイン」、リゾルブが「ステージ」のアクションを実行します。  
    詳細は :numref:`trigger-act-n` をタイミング=メインとして参照してください。

**[6-11] タイミング=クイックのステージアクションを処理**  
    同じプレイヤーについて、タイミングが「クイック」、リゾルブが「ステージ」のアクションを実行します。  
...
誘発チェックで誘発したリゾルブが即時のリクエストを処理します。
...
**[6-5-1] バッファから該当の即時はあるか？**  
    バッファから対象プレイヤーかつ、該当するタイミングかつ、リゾルブが即時のリクエストが存在するかを判定します。
...
誘発チェックで誘発したリゾルブがステージのリクエストを処理します。
呼び出し元で指定されたプレイヤーおよびタイミングに基づいて処理します。
...
.. uml:: triggerflow_n.puml
    :caption: 誘発チェック-ステージ処理
    :scale: 50%


**[6-10-1] バッファから該当のリゾルブ=ステージはあるか？**  
    バッファから対象プレイヤーかつ、該当するタイミングかつ、リゾルブがステージのリクエストが存在するかを判定します。
    存在する場合、以降の処理へ進み、存在しなくなるまでこのループを継続します。
