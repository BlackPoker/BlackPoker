...
フローチャートで示すと次のようになります。(:numref:`action-request-flow-image`)

.. _action-request-flow-image:
.. uml:: action-request-flow.puml
    :caption: アクションの起こし方のフロー
    :scale: 50%
...
アクションのスピードによって解決の流れが異なります。
主な違いは次の通りです。(:numref:`table-speed-compare`)

.. _table-speed-compare:
.. table:: スピードによる解決フローの比較

    +------------+--------------------------+-----------------------+--------------------------+
    | スピード   | ステージ（スタック）     | 割り込みの可否        | 解決のタイミング         |
    +============+==========================+=======================+==========================+
    | **通常**   | 使用する（積まれる）     | **可能**              | 全員がパスした後         |
    +------------+--------------------------+-----------------------+--------------------------+
    | **即時**   | 使用しない（積まれない） | **不可**              | リクエストした直後       |
    +------------+--------------------------+-----------------------+--------------------------+
...
.. seealso::
   ...
   この省略進行の指針については :numref:`shortcut-guideline` を参照してください。
...
相手がさらにアクションを積むか、アクションを積まずに **パス** するかを選択します。
...
解決の流れを次の図に示します。(:numref:`action-resolve-immediate-image`)

.. _action-resolve-immediate-image:
.. uml:: action-resolve-immediate.puml
    :caption: 即時アクションの解決フロー
    :scale: 50%

即時の場合は、リクエストされた時点でルールシステムがすぐにアクションを解決し、
全プレイヤーに結果が反映されます。相手はそのアクションに対して割り込む
（カウンターなど）ことができません。
...
.. seealso:: 次に説明する部分は、 :numref:`coreflowsec` の :ref:`actresolve` で順に行われます。
...
フローチャートで示すと次のようになります。(:numref:`action-resolve-flow-image`)

.. _action-resolve-flow-image:
.. uml:: action-resolve-flow.puml
    :caption: リクエスト解決のフロー
    :scale: 50%
