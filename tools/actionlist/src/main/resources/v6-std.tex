...
\hrule height 0.5mm depth 0mm width 66.5mm %罫線
\vspace{1mm} %余白追加
#if((${arg0} == 'lite') && (${row.format.contains('lite')}) && (${row.format.contains('std')}))
{\small\bf ■ ${row.actName} {\scriptsize (${row.actType}) }} %Actionタイトル
#elseif((${row.format.contains('lite')}) && (${row.format.containts('std')}))
{\small\bf ■ ${row.actName} {\scriptsize (${row.actType}) [lite]}} %Actionタイトル
#elseif(${row.format.containts('std')})
