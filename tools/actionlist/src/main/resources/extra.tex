...
#if(!$acts.type.startsWith("切札"))
#foreach( $row in ${acts.acts})
#if(${row.frame} && ${row.frame.contains("エクストラ")})
%%% Action %%%
%\vspace{1zh} %余白追加
\vspace{2mm} %余白追加
\hrule height 0.5mm depth 0mm width 66.5mm %罫線
...
\hrule height 1mm depth 0mm width 66.5mm %罫線
\vspace{1mm}%余白削除
{\Large\bf \ruby{Trump List}{切札リスト}}
\vspace{1mm}%余白削除
\hrule height 0.5mm depth 0mm width 66.5mm %罫線
\end{center}
\end{center}
...
#foreach( $row in ${nestList.trumps})
#if(${row.frame} && ${row.frame.contains("エクストラ")})
...
\hrule height 0.5mm depth 0mm width 66.5mm %罫線
\vspace{1mm} %余白追加
{\Large\bf ${row.trumpKey}} {\normalsize\bf【${texFn.cnv(${row.trumpName})}】} %Trumpタイトル
\vspace{1mm} %余白追加

${texFn.cnv(${row.trumpAbility})}

#set($rel = $trumpRelations.get($nestList.type))
%%%%% Trumpアクション %%%%%
#if($rel && $rel.acts && $rel.acts.size() > 0)
#foreach($act in $rel.acts)
#if(${act.frame} && ${act.frame.contains("エクストラ")})
\begin{tcolorbox}[title={\small\bf【Action】${act.actName}}{\scriptsize （${act.actType}）}]

#if(${act.actTime} != "")
{\scriptsize\bf @${act.actTrigger}-${act.actSpeed}-${act.actTime} }
#end
#if(${act.actCost} != "")
...
#end
#if(${act.actUnit.trim()} != "")
  {\scriptsize\bf | ★${texFn.cnv(${act.actUnit})}}
#end
...
${texFn.cnv(${act.actActCond})}

#end
#if(${act.actTriggerCond})
\vspace{1mm} %余白追加  
\hrule height 0.1mm depth 0mm width 62mm %罫線
\vspace{1mm} %余白追加
...
\vspace{-1zh}
#end ##エクストラ frame判定 end
#end ##foreach end
#end ##Trumpアクション end 

%%%%% Trumpキャラクター %%%%%
#if($rel && $rel.chars && $rel.chars.size() > 0)
#foreach($char in $rel.chars)
#if(${char.frame} && ${char.frame.contains("エクストラ")})
\vspace{2mm}
...
#end
#if(${char.charSize.trim()} != "")
  {\scriptsize\bf | サイズ : ${texFn.cnv(${char.charSize})}}
#end
...
${texFn.cnv(${char.charNote})}
\vspace{1zh}%余白追加
#end
#if(${char.charLabel.trim()} != "")
\vspace{1mm} %余白追加
\hrule height 0.1mm depth 0mm width 62mm %罫線
\vspace{1mm} %余白追加
