...
export class ViewerAwareGameEventFormatter {
  /**
   * 前後の State と解決アクション情報から、閲覧者視点に合わせたイベントログ配列を生成します。
    ...
    for (const pKey of ["p1", "p2"]) {
      ...
      if (prevLife > nextLife) {
        ...
        if (diff > 0) {
          ...
          // 相手のライフ表示: 10以上なら "10以上" とし正確枚数を秘匿
          ...
          if (nextHand > prevHand) {
            ...
            logs.push({
              message: `[DRAW] ${pName} がライフから ${drawCount}枚 ドローしました (残りライフ: ${remainingLifeDisplay})`,
              level: "event",
            });
          } else {
            logs.push({
              message: `[DAMAGE] ${pName} に ${diff} ダメージ！ (残りライフ: ${remainingLifeDisplay})`,
              level: "event",
            });
          }
        }
