...
- **一時的戦闘状態 `state.combat` のデータモデル設計**:
  - BlackPoker に存在しない概念を増やさず、GameState 内に軽量な一時オブジェクト `state.combat` を定義して、アタッカーやブロックの対応関係を格納し、ダメージ判定解決後にクリアするシンプルな設計としました。
...
## 31. Phase 14.1 検証結果: アタックアクションの最小実装と戦闘状態モデルの追加

本セクションでは、Phase 14.0で策定した戦闘モデルに基づき、アタックアクションのYAML定義、タイミング検証、アタッカー指定のバリデーション、および解決時の戦闘一時状態（`state.combat`）構築とアタッカーのドライブ移行処理を最小実装・検証した結果について記述します。
...
- **戦闘一時状態（`state.combat`）の構築コマンド（`startAttack`）の実装**:
  ...
  - アタックアクション解決時に以下の処理を実行します。
    ...
    2. **戦闘一時状態の構築**: `state.combat` を作成し、アタッカーID（`attackerUnitId`）、攻撃プレイヤー（`attackerPlayerKey`）、防御プレイヤー（`defenderPlayerKey`）、解決中のリクエストID、ステータス（`"attacking"`）を格納します。
...
- **アタックアクション専用の統合テストを追加 (`attack.test.ts`)**:
  - [attack.test.ts](file:///c:/Users/black/git/github/BlackPoker/tools/simulator/src/tests/rules-vnext/attack.test.ts) を新規作成し、以下の振る舞いを網羅的にアサート検証しました。
    ...
    2. アタック解決時に、戦闘一時状態 `state.combat` が正しく生成され、`attackerUnitId` / `attackerPlayerKey` / `defenderPlayerKey` が正常に記録されること (B, C)。
...
- **CLIシナリオランナーへの統合と実行 (`npm run scenario:rules-vnext`)**:
  ...
  - アタック解決時に `[CMD] startAttack` -> `[COMBAT] attacker=一般兵, defender=Player B` の美麗なログがフック出力され、戦闘一時状態が正しくコンソールに表示されて正常に完走することを確認しました。

### 今後のステップ

本フェーズでアタックによる戦闘状態モデルの構築まで完了しました。次の **Phase 14.2** では、防御側がブロッカーを指定する **「ブロックアクション（`action.block`）」** の最小実装に進む予定です。
