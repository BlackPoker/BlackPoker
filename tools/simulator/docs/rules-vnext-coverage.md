...
| `attack` | アタック | `action.attack` | `act.yaml` | 未実装（設計整理済み） | - | なし | なし | Phase 14.0 にて戦闘モデルの設計整理（アクションベースの解決連鎖、一時状態モデル、ロードマップ構築）を完了。Phase 14.1〜で実装予定。 | **高** |
...
| **アタック処理** | `公式ルール本文` | 未実装（設計整理済み） | なし | なし | Phase 14.0 にて設計整理完了。アタッカーの指定条件バリデーションや一時状態 `state.combat` へのバインド設計を構築。 | **高** |
