...
| `attack` | アタック | `action.attack` | `act.yaml` | 最小実装済み | OK | あり | あり | Phase 14.1/14.1.1にて、アタッカー指定バリデーション、ドライブ状態移行、およびアタッカー側への戦闘一時情報（unit.battle）バインドを最小実装完了。 | **高** |
| `block` | ブロック | `action.block` | `act.yaml` | 最小実装済み | OK | あり | あり | Phase 14.2にて、ブロッカー指定バリデーション、内部タイミング timing: block、アタッカー厳密自動特定、および battle 一時情報による対応付けを最小実装完了。 | **高** |
| `damageJudge` | ダメージ判定 | `action.damageJudge` | `act.yaml` | 最小実装済み | OK | あり | あり | Phase 14.3にて、アタッカー・ブロッカーサイズ比較（フォグ補正込み）、敗北側墓地送り（カード個別cardMoved発行）、世代交代連携、および battle 戦闘一時情報の完全クリアを最小実装完了。 | **高** |
...
| **アタック処理** | `公式ルール本文` | 最小実装済み | あり | あり | Phase 14.1 にて最小実装完了。アタッカー指定条件バリデーション（所有権・チャージ状態判定）、ドライブ状態への移行、およびアタッカー戦闘一時情報（unit.battle）の記録を実装・検証完了。 | **高** |
| **ブロック処理** | `公式ルール本文` | 最小実装済み | あり | あり | Phase 14.2 にて最小実装完了。防御側によるブロックアクション（action.block）の定義、タイミング timing: block の導入、自分を狙う相手側アタッカー自動特定、およびブロッカー戦闘一時情報（unit.battle）によるアタッカーとの対応関係バインドを実装・検証完了。 | **高** |
...
| `declareBlock` | ブロック宣言 | `rules-vnext独自` | 完了 | あり | あり | ブロッカーのドライブ移行とブロッカー戦闘一時情報（unit.battle）の記録 |
