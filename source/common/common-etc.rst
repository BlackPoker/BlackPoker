...
各カードの配置と公開・非公開の度合いは :numref:`common-etc-publevel-table` のとおりです。

.. _common-etc-publevel-table:

.. csv-table:: 各カードの配置と公開・非公開の度合い
   :widths: 15, 30, 30, 25
   :header-rows: 1
   :file: ./public-level.csv


墓地の一番上のカードの扱い
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
墓地の一番上にあるカードは常に公開（表向き）とします。

* **カードを墓地へ移すとき**:
  複数のカードを同時に墓地へ移す場合は、移動するカードの中から任意の1枚を選んで一番上に置いて公開してください。
* **墓地からカードを別の領域に移したとき**:
  墓地に残ったカードの中から任意の1枚を選んで一番上に置くことができます。
