...
その場所を領域（ゾーン）と呼びます。
各領域には、その領域におけるカードの種類が定義されています。
その定義をコンポーネントと呼びます。
また、実際に置かれている兵士や防壁などのカードの塊をユニットと呼びます。
ユニットを構成するカードをユニットカードと呼びます。

次の表は領域毎に存在するコンポーネントの一覧です。



び、以下の属性で管理します。  
また、同じユニット（キャラクター）に紐づくカード群をまとめる「ユニットカード」という概念も、このゾーン管理の一部として扱います。

.. csv-table:: ゾーンとカードの状態
   :widths: 1, 1, 1, 1, 1, 1
   :header-rows: 1

    区分,ゾーン,カテゴリー,タイプ,カードの状態,公開レベル
    兵士,"場（オモテ）","キャラクター","兵士／防壁","表向き","完全公開"
    防壁,"場（ウラ）","キャラクター","兵士／防壁","裏向き","個人公開"
    速攻魔法,"場（オモテ）","アクション","速攻魔法","表向き","完全公開"
    通常魔法,"場（オモテ）","アクション","通常魔法","表向き","完全公開"
    切札,"場（オモテ）","切札","エクストラ","表向き","完全公開"
    隠し札,"場（ウラ）","切札","エクストラ","裏向き","個人公開"
    未開封パック,-,"パック","開封前","裏向き","非公開"
    開封済パック,-,"パック","開封後","表向き（一番上のみ完全公開）","個人公開／完全公開"
    レアカード,-,"特殊カード","レアカード","裏向き","個人公開"
    手札,"カード","手札","プレイヤー手元","裏向き","個人公開"
    ライフ,"カード","ライフ","山札","裏向き","非公開"
    墓地,"カード","墓地","捨て札置き場","表向き（一番上のみ完全公開）","個人公開／完全公開"
    ステージ,"リクエスト","ステージ","アクション蓄積場所","傾けて置く","完全公開"
    システム,-,"システム処理","ゲーム進行中（仮想領域）","不定","完全公開"

※ 開封済パックの表向きは「一番上のカードのみ完全公開」、墓地の表向きも「一番上のカードのみ完全公開」です。

ユニットカード
------------------------------
ユニットカードは、「ゾーン」に置かれるキャラクターカードやフォグカードなど、同じユニット（キャラクター）に紐づくカード群の“親”にあたるカードです。  
ひとつのユニットを構成するカードをまとめて管理し、以降の「カードの配置」や「アクション」の説明では、このユニットカードを起点に各種カードの振る舞いを理解してください。
